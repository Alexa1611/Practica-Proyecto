...
class Parser:
    ...
    def parse(self):
        ...
        if not self.HErrores and not self.preanalisis == self.finCadena:
            print("Error en la posición " + str(self.preanalisis.posicion)+ ". No se esperaba el token " + str(self.preanalisis.tipo))
    ...
    def d(self):
        ...
        if self.preanalisis == self.distinct:
            ...
            self.HErrores = True
            print("Error en la posición " + str(self.preanalisis.posicion) + ". Se esperaba DISTINCT, * o un identificador.")
    def p(self):
        if self.HErrores:
            return
        self.a()
    ...
    def pp(self):
        if self.preanalisis == self.coma:
            self.match(self.coma) #manda llamar al token ,
            self.p()
    ...
    def a(self):
        ...
        if self.preanalisis == self.asterisco:
            ...
            self.HErrores = True
            print("Error en la posición " + str(self.preanalisis.posicion) + ". Se esperaba un identificador.")
    ...
    def a1(self):
        ...
        if self.preanalisis == self.punto:
            ...
            self.a1
    def t(self):
        if self.HErrores: #analiza que no haya errores si hay errores NO HACE NADA
            return
    ...
    def match(self, t):
        ...
        if self.preanalisis.tipo == t.tipo:
            self.i += 1 # estado, si se encuentra i aumenta 1
            self.preanalisis = self.tokens[self.i]
        else:
            self.HErrores = True
